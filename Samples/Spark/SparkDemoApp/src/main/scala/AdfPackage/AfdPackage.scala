...
object AdfPackage
{
    ...
    {
        val conf = new SparkConf().setAppName("Simple Spark ADF With Package")
        val sc = new SparkContext(conf)
        val sqlContext = new SQLContext(sc)

        val taxiFareRdd = sc.textFile("wasb://e2e@***.blob.core.windows.net/day_03.csv")
        ...
        taxiFareDf2.write.format("com.databricks.spark.csv").option("header", "true").save("wasb://output@***.blob.core.windows.net/day_03.csv")
    }
}