...
// Merge new data into existing hive table
object AdfHiveTable2
{
    def main(args: Array[String]) 
    {
        val conf = new SparkConf().setAppName("Adf Hive Table2")
        val sc = new SparkContext(conf)
        val sqlContext = new HiveContext(sc)

        val taxiFareRdd = sc.textFile("wasb://e2e@***.blob.core.windows.net/day_02.csv")
        ...
        taxiFareDf.registerTempTable("fareDay")
        sqlContext.sql("INSERT INTO AdfFareTable SELECT * from fareDay")
        sqlContext.dropTempTable("fareDay")
    }
}